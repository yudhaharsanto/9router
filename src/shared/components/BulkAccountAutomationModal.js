        ...
        {!activeJob && (
          <>
            ...
            <div>
                    ...
                    <div className="rounded-lg border border-border bg-background">
                      <div className="flex items-center gap-2 border-b border-border px-2 py-1.5">
                        ...
                        <input
                          ...
                          value={proxySearch}
                          onChange={(event) =>
                            setProxySearch(event.target.value)
                          }
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            const allFilteredSelected = filteredIds.every(
                              (id) => proxyPoolIds.includes(id),
                            );
                        ...
                        >
                          ...
                          filteredPools.every((p) =>
                            proxyPoolIds.includes(p.id),
                          )
                            ? "Unselect All"
                            : "Select All"}
                        </button>
              ...
              <p className="mt-1 text-xs text-text-muted">
                Browsers will route Google login traffic through the chosen
                proxy. Select multiple pools to rotate per account
                (round-robin). Relay-style pools (Vercel, Cloudflare, Deno) are
                excluded because they only rewrite API URLs.
              </p>
            </div>
          </>
        )}
          ...
          <div className="space-y-4">
            ...
            {activeJob.summary?.needs_manual > 0 && (
              <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800 dark:border-amber-800 dark:bg-amber-900/20 dark:text-amber-200">
                Some accounts need manual assist. Open the worker session,
                finish the Google or {serviceName} prompts, and the job will
                keep polling.
              </div>
            )}
                    ...
                    <div className="grid gap-3 xl:grid-cols-2">
                      {group.accounts.map((account) => (
                        ...
                        >
                          ...
                          {account.manualSessionAvailable &&
                          account.workerId ? (
