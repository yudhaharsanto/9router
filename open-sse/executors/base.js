  ...
  async execute({
    ...
    for (let urlIndex = 0; urlIndex < fallbackCount; urlIndex++) {
      ...
      try {
        ...
        // Rate-limited through a pooled proxy: rotate to another proxy and
        // retry transparently (once) so the client never sees the 429.
        if (
          response.status === HTTP_STATUS.RATE_LIMITED &&
          typeof rotateProxy === "function" &&
          !proxyRotated
        ) {
          proxyRotated = true;
          const next = await rotateProxy(proxyOptions, url);
          if (next) {
            proxyOptions = next;
            log?.debug?.("RETRY", `429 on ${url}, rotating proxy → retry`);
            urlIndex--;
            continue;
          }
        }
