...
describe("opencode Free 1.3 tool_choice auto-only", () => {
  it("khai quirk đúng model 1.3-Free trong registry", () => {
    expect(PROVIDERS.opencode.quirks?.forceAutoToolChoiceModels).toEqual([
      FREE_12,
      FREE_13,
    ]);
  });
  ...
  it.each([
    ...
    ["required", "required"],
    ["none", "none"],
  ])("demote %s sang auto (plain dan max)", (_label, choice) => {
  ...
  it("giữ auto; set auto bila absent; tools cloak selalu", () => {
    ...
    expect(autoOut.tools.map((t) => t.name)).toEqual([
      ...TOOLS.map((t) => t.name),
      ...DECOY,
    ]);
    ...
    expect(absentOut.tools.map((t) => t.name)).toEqual([
      ...TOOLS.map((t) => t.name),
      ...DECOY,
    ]);
  ...
  it("wire: execute gửi choice auto tới /zen/v1/responses", async () => {
    ...
    const body = responsesBody(FREE_13, {
      type: "function",
      name: "get_weather",
    });
