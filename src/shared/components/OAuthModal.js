...
export default function OAuthModal({
  ...
  const startOAuthFlow = useCallback(
    async (authMethod = "google", captchaVerifyParam = null) => {
      if (!provider) return;
      try {
        setError(null);
        ...
        if (deviceCodeProviders.includes(provider)) {
          ...
          // AutoClaw: captcha verify param is mandatory for the OAuth URL request.
          if (provider === "autoclaw") {
            const autoclawUrl = new URL(
              `/api/oauth/autoclaw/device-code`,
              window.location.origin,
            );
            autoclawUrl.searchParams.set("auth_method", authMethod || "google");
            if (captchaVerifyParam) {
              autoclawUrl.searchParams.set("captcha_param", captchaVerifyParam);
            }
            const res = await fetch(autoclawUrl.toString());
            const data = await res.json();
            if (!res.ok) throw new Error(data.error);
  ...
  // AutoClaw: server requires Aliyun captcha verify param before issuing the
  // OAuth URL. Load the SDK once, then reuse the popup instance.
  const autoclawMethodRef = useRef("google");
  const ensureCaptchaScript = () =>
    new Promise((resolve, reject) => {
      if (window.initAliyunCaptcha) {
        resolve();
        return;
      }
      const existing = document.getElementById("aliyun-captcha-sdk");
      if (existing) {
  ...
  const startAutoclawLogin = useCallback(async () => {
    try {
      await ensureCaptchaScript();
      if (window._autoclawCaptchaInstance) {
        window._autoclawCaptchaInstance.show?.();
        return;
      }
      window.initAliyunCaptcha({
        ...
        region: "ga",
        mode: "popup",
        element: "#autoclaw-captcha-element",
        button: "#autoclaw-captcha-button",
        language: "en",
        captchaVerifyCallback: async (captchaVerifyParam) => {
          try {
            await startOAuthFlow(autoclawMethodRef.current, captchaVerifyParam);
          } catch (err) {
            setError(err.message);
            setStep("error");
          }
          // Captcha itself succeeded — returning false would make the SDK
          // reset and re-show the popup on business-layer failures.
          return { captchaResult: true };
        },
        onBizResultCallback: () => {},
        getInstance: (instance) => {
          window._autoclawCaptchaInstance = instance;
          instance.show?.();
        },
      });
    } catch (err) {
    ...
    <Modal isOpen={isOpen} title={modalTitle} onClose={handleClose} size="lg">
      <div className="flex flex-col gap-4">
        ...
        {provider === "autoclaw" &&
          !(
            isDeviceCode &&
            (step === "waiting" || step === "input" || step === "success")
          ) && (
            <div className="space-y-3">
              <p className="text-sm text-text-muted">
                Selesaikan verifikasi captcha, lalu login Google/Z.ai akan
                terbuka di browser.
              </p>
              <div id="autoclaw-captcha-element" />
              <div id="autoclaw-captcha-button" className="hidden" />
              <div className="flex gap-2">
                <Button
                  onClick={() => {
                    autoclawMethodRef.current = "google";
                    setError(null);
                    startAutoclawLogin();
                  }}
                  fullWidth
